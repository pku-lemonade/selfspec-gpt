...
VERIFY_DELTA_DAC_BITS="${VERIFY_DELTA_DAC_BITS:-8}"
DRAFT_DELTA_DAC_BITS="${DRAFT_DELTA_DAC_BITS:-8}"
...
run_sweep() {
  local model_tag="$1"
  local draft_adc_bits="$2"
  local checkpoint_path="$3"
  local out_json="out/k_sweep_${model_tag}_delta_draftadc${draft_adc_bits}_verifyadc${VERIFY_ADC_BITS}_deltadac${VERIFY_DELTA_DAC_BITS}_draftdeltadac${DRAFT_DELTA_DAC_BITS}_s100.json"
  ...
  echo "[run] model=${model_tag} draft_adc_bits=${draft_adc_bits} verify_adc_bits=${VERIFY_ADC_BITS} verify_delta_dac_bits=${VERIFY_DELTA_DAC_BITS} draft_delta_dac_bits=${DRAFT_DELTA_DAC_BITS}"
  python scripts/sweep_speculate_k.py \
    ...
    --verify_delta_dac_bits "$VERIFY_DELTA_DAC_BITS" \
    --draft_adc_bits "$draft_adc_bits" \
    --draft_delta_readout \
    --draft_delta_dac_bits "$DRAFT_DELTA_DAC_BITS" \
    --out_json "$out_json"
}
...
for task in "${TASK_LIST[@]}"; do
  ...
  run_sweep "$model_tag" "$draft_adc_bits" "$checkpoint_path"
done

echo "[done] full delta ADC-by-k matrix queue finished"
