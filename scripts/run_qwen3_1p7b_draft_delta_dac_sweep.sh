...
run_sweep() {
  local dac_bits="$1"
  local out_json="out/compare_qwen3_1p7b_adc${DRAFT_ADC_BITS}_k${K_VALUES// /_}_verify${VERIFY_ADC_BITS}_draftdelta_dac${dac_bits}.json"
  ...
  echo "[run] model=qwen3_1p7b draft_adc_bits=${DRAFT_ADC_BITS} draft_delta_dac_bits=${dac_bits} verify_adc_bits=${VERIFY_ADC_BITS} k_values=${K_VALUES}"
...
echo "[done] qwen3_1p7b draft delta-DAC sweep queue finished"
